  ...
  if (Array.isArray(obj)) {
    ...
    if (scope)
      Object.keys(obj).forEach((key) =>
        serialize(params, obj[key], `${scope}[${key}]`)
      );
    else Object.keys(obj).forEach((key) => serialize(params, obj[key], key));
  } else {
...
export default class Ajax {
  ...
  request(options) {
    return new Promise((resolve, reject) => {
      ...
      if (hashIndex > -1) setting.url = setting.url.slice(0, hashIndex);
      ...
      if (setting.contentType) setHeader("Content-Type", setting.contentType);
